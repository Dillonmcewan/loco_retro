...
- **R6 — Dot voting.** During the *Vote* phase, each participant has N votes (default 5, configurable by the facilitator at room creation) to allocate across cards, with an optional **"Chris mode"** at room-creation time that removes the per-participant cap entirely (uncapped voting; participants signal "I'm done" manually). Aggregate vote counts are visible to everyone; individual ballots are private.
...
- **R8 — Local-first sync.** Each client holds full retro state locally via a CRDT and syncs through a Cloudflare Durable Object (via `y-partyserver`, running in our own Cloudflare Workers account). Edits made while offline appear locally immediately and merge on reconnect without data loss, for the duration of a single retro session.
...
- ~~Does the relay persist room state when no participants are connected, or is it pure pass-through?~~ **Resolved:** the Durable Object holds the latest Yjs snapshot in its default SQLite storage while the room is active. Cloudflare manages the DO storage; we operate the Worker but write no persistence code of our own.
