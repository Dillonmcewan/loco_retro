...
- **Interactive card surfaces — compose, don't reinvent.** Every clickable card-shaped surface (dashboard tiles, template pickers, "add new" CTAs) is built on `CardSurface.svelte` so hover-lift, focus-visible ring, active, and reduced-motion behavior stay consistent. The shared `--card-*` tokens in `src/app.css` (`--card-radius`, `--card-border-width`, `--card-shadow-rest`/`-hover`, `--card-lift`, `--card-accent`, `--card-transition`) are the single source of truth — never re-declare `.card` hover styles in a consumer. **Pick the right primitive:** `CardSurface` for plain clickable surfaces (CTAs, navigation tiles); `SelectableCard` for items in a "pick one" group (handles the `selected` peach-fill highlight and suppresses hover on the selected card). Override the accent per-consumer with `style="--card-accent: var(--color-phase-X)"` (see `RoomTile.svelte` for the phase-color pattern). Content cards (`RetroCard.svelte`) are a separate visual category and stand outside this system.
